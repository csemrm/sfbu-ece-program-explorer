...
- [x] Fix: 12 catalog-stated prerequisites were missing. `PREREQUISITES` is generated from the catalog PDF and was not regenerated when the 49 Fall 2026 courses were added, so they shipped prerequisite-free — CS547 had none while CS550, whose catalog entry states the same `Prerequisite: CS457`, had one. Several EE links (EE504/EE505/EE511/EE520/EE577 → EE461, EE553 → EE488) had also been dropped earlier as unresolvable, because only the G-suffixed variants were seeded at the time. Now 59 links; `catalog-data.spec.ts` spot-checks eleven of them against the printed catalog and asserts a floor on the total.
- [ ] **`PREREQUISITES`, `COREQUISITES` and `COURSE_KNOWLEDGE_AREAS` must be regenerated whenever `COURSES` grows.** Referential integrity cannot catch a missing link — an absent row is perfectly consistent — so adding a course without its relationships fails silently. This has now happened twice.
...
- [x] Fix: `PlannerTermDto.courseIds` was capped at 30, sized for a student's course load. The planner evaluates *every* offered course so it can show what is blocked before the student commits, so the real bound is a term's full schedule — once Fall 2026 was seeded in full and electives were placed, all three degrees exceeded it (BSCS 55, MSCS 36, MSEE 31) and `/plan` returned 400 for every degree. Raised to 200 and pinned by `planner.dto.spec.ts`.
...
- [x] **Disjunctive prerequisites are modeled.** `prerequisites.alternative_group` (migration `DisjunctivePrerequisites1719446406000`) groups interchangeable rows: satisfying any one satisfies the group, while separate groups and null-group rows stay ANDed, so every pre-existing row keeps its meaning. Five courses use it — CS480 (CS250 or CS360), CS480L (CS250L or CS360L), CS556 (CS360 or CS500), DS520 and DS565 (CS500 or DS501) — 10 rows in 5 groups, previously omitted entirely and so appearing prerequisite-free. `missingPrerequisites[].alternativeGroup` carries it to the client, and both the `reason` string and the planner UI read "CS360 or CS500" as one requirement.
...
- [x] MSDS is modelled. The School of Engineering offers it alongside MSCS and MSEE, and its whole curriculum is in the catalog: Foundation (CS481G, DS500, DS501), the 12-credit Data Science specialization, 6 credits of electives and the DS595 capstone — 30 credits. Seven DS courses were added; DS500 and DS520 were already seeded from the Fall 2026 schedule.
- [ ] `ProgramCard` keys degree label, credit total, accent and badge colour off four hand-maintained maps of programme abbreviations. A new degree renders without them until it is added by hand — which MSDS was. CLAUDE.md asks for new programmes without redesign, so these should come from the API (credits are already derivable from the roadmap).
- [ ] **DS535 "Large Language Models (LLM)" has no course description or credit hours in the catalog** — it appears only in the MSDS specialization table. Seeded at 3 credits, taken from its siblings in the same 12-credit requirement, and flagged in its description. Needs departmental confirmation.
- [ ] The catalog also documents BSBA/MBA/MSBAn business programs and a Graduate Certificate in Management that this app does not model. Their course codes are seeded (as electives) but the programs are not.
